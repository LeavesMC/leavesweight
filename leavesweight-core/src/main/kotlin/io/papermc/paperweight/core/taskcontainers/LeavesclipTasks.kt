...
class LeavesclipTasks(
    ...
    private fun Project.createTasks(
        classifier: String = "",
    ): Pair<TaskProvider<CreateBundlerJar>, TaskProvider<CreateLeavesclipJar>> {
        ...
        val paperclipJarTask = tasks.register<CreateLeavesclipJar>(paperclipTaskName) {
            group = "bundling"
            description = "Build a runnable leavesclip jar"

            libraryChangesJson.set(bundlerJarTask.flatMap { it.libraryChangesJson })
            outputZip.set(layout.buildDirectory.file(jarName("leavesclip", classifier).map { "libs/$it" }))
        }
    ...
    private fun TaskProvider<CreateBundlerJar>.serverJar(
        ...
        serverLibrariesList.set(this@LeavesclipTasks.serverLibrariesList)
    ...
    private fun TaskProvider<CreateLeavesclipJar>.bundlerJar(
        createBundlerJar: TaskProvider<CreateBundlerJar>,
        op: Action<CreateLeavesclipJar> = Action {},
    ) = configure {
        originalBundlerJar.set(vanillaJar)
        bundlerJar.set(createBundlerJar.flatMap { it.outputZip })
        mcVersion.set(this@LeavesclipTasks.mcVersion)
